...
### Performance anti-patterns (baseline) — fixed on this branch
...
The baseline (`main` branch) collapses under load; this branch (`axelix-applied`),
with the fixes, does not. Numbers below are from a single host — absolute values
...
| Endpoint | Root cause on `main` | Effect | Fix on this branch |
|--|--|--|--|
...
Beyond the JPA fixes, this branch's image runs the JVM with two footprint features
(see the `Dockerfile` entrypoint): **compact object headers**
(`-XX:+UseCompactObjectHeaders`, Project Lilliput — 8-byte object headers) and an
**AOT class-loading/linking cache** (`-XX:AOTCache=app.aot`, Project Leyden, built
during the image build via the layered extraction). Measured on the same image and
code, same `-Xmx1g`, under an identical moderate steady load (~52 req/s of the read
endpoints, 0 errors on both runs), toggling the two flags together (neither vs both):

| RSS at… | Neither flag | Both (COH + AOT) | Reduction |
|--|--:|--:|--:|
