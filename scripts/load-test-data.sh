...
#
# Options are forwarded to load_test_data.py (e.g. --seed 123, --create-schema).
...
# Ensure schema exists via a one-shot app start (Hibernate ddl-auto=update).
...
if [[ "${TABLE_COUNT:-0}" == "0" ]]; then
  ...
  if ! curl -sf http://localhost:8080/actuator/health >/dev/null 2>&1; then
    echo "App did not become healthy; run with --create-schema instead:" >&2
    echo "  python scripts/load_test_data.py --create-schema" >&2
    exit 1
  fi
...
PGHOST=localhost PGPORT=5432 PGDATABASE=orderdb PGUSER=orderuser PGPASSWORD=orderpass \
  "$VENV/bin/python" scripts/load_test_data.py --create-schema "$@"
